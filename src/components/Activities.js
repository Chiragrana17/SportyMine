...
const activities = [{
    ...
    },
    {
        ...
        description: "Creative ways to stay active, from circuit challenges to team relays that boost energy and excitement.",
        image: "/ffg.jpg",
    },
    {
        title: "Confidence Building",
        description: "We focus on individual progress, encouraging sportsmanship, leadership, and personal achievement.",
        image: "/cb.jpg",
    },
    {
...
const Activities = () => {
    return ( <
        section className = "after-school-section"
        id = "after-school" >
        <
        h2 className = "after-heading" > After - School Activities Program < /h2>{" "} <
        p className = "after-subtext" >
        Once the school day ends, the fun continues with our After - School Program— a high - energy, skill - building experience that keeps children moving, learning, and laughing. { " " } <
        /p>{" "} <
        div className = "after-program-list" > { " " } {
            activities.map((item, index) => ( <
                div className = { `after-program-row ${index % 2 !== 0 ? "reverse" : ""}` }
                key = { index } >
                <
